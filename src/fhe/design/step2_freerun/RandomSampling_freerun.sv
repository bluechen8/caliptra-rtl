...
module RandomSampling #(
  ...
  logic random_valid;
  // C'-2: the Trivium free-runs; it is reset only on prng_rst (power-on) and
  // reloaded only on `reseed` -- NOT on the per-pass `rst`. This lets the
  // keystream continue across sampling passes (fresh a/e0 that never repeat)
  // while keygen can still reseed deterministically from the KV/keygen seed.
