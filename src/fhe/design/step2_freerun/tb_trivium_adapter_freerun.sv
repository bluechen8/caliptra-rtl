`timescale 1ns / 1ps
// C'-2 debug/unit TB: isolate TriviumAdapter (free-run caliptra_prim_trivium).
// Verifies:
//   (1) reseed(seedA) -> random_valid rises, keystream = CaliptraPrimTrivium(A) w0..
...
module tb_trivium_adapter;
  ...
  TriviumAdapter dut(.clk(clk), .rst(rst), .reseed(reseed), .seed(seed),
