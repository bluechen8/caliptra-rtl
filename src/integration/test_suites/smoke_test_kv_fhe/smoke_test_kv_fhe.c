...
// C'-1c: KeyVault entry the TB backdoor (0xc6) provisions with the keygen seed.
// The injected value (fhe_kv_seed_tb in caliptra_top_tb_services) is
// 0xDEADBEEF_CAFEBABE; the TB asserts the walker's keygen seed matches it.
#define FHE_KV_ENTRY     5u
...
void main(void) {
    VPRINTF(LOW, "----------------------------\n");
    VPRINTF(LOW, " Running FHE KeyVault Smoke Test !!\n");
    VPRINTF(LOW, "----------------------------\n");
    ...
    // 4) Program the microsequencer. C'-1c: the keygen root seed comes from the
    //    KeyVault, NOT the KGSEED registers. Provision the seed into KV entry
    //    FHE_KV_ENTRY via the TB backdoor (opcode 0xc6, authorizes read-client 6
    //    = the FHE seed client), then point KGKV_CTRL at it and enable KV.
    lsu_write_32(STDOUT, (FHE_KV_ENTRY << 8) | 0xc6);  // TB: inject KV seed
    fhe_set_kgkv(FHE_KV_ENTRY, /*en*/ 1);
    ...
    check32("KGKV_CTRL rdbk", lsu_read_32(FHE_REG_KGKV_CTRL), ((FHE_KV_ENTRY & 0x1Fu) << 4) | 1u);
